...
output: rticles::elsevier_article # word_document #   #rticles::plos_article
...
```{r Precipitation, echo=FALSE, fig.height=12, fig.width=5, message=FALSE, warning=FALSE}
# Spatial Plots -----------------------------------------------------------
library(rasterVis)
library(gridExtra)
# Drought Plots
...
```{r Panel Regression, echo=FALSE, message=FALSE, warning=FALSE}
...
#### Panel Regression ----------------------------------------------------
 ...
 # read data
  ...
  form_1_maz_z =  MAIZEOPH_W ~ lag(data_in_plm$MAIZEOPH_W, 1) +G_mn +  G_AUC_leading + G_AUC_Qnt  + 
    ...
    MAIZEDAMAGEAREA_P + MAIZEFERT_CHEMICAL_AREA + MAIZEFERT_CHEMICAL_AMT + 
     MAIZEEXTAREA +   Y_COORD + X_COORD + elevation + I(as.numeric(Year)) +
     + factor(Z_CODE)
  
  maz.re1 <- plm(form_1_maz_z, data = data_in_plm, model = "random")
  # summary(maz.re1)
  
  form_2_maz_z = MAIZEOPH_W ~ lag(data_in_plm$MAIZEOPH_W, 1)  + 
    A_Qnt   + A_AUC_Qnt + T_G_Qnt  + MAIZEIMSEED +
    MAIZEDAMAGEAREA_P + MAIZEAREA +  I(MAIZEFERT_CHEMICAL_AMT/(MAIZEFERT_CHEMICAL_AREA+1)) +
    MAIZEEXTAREA + PPT_A_max + PPT_G_AUC_Qnt + PET_G_AUC_diff_mn   + 
    I(as.numeric(Year)) + Y_COORD +  X_COORD + bs(elevation,3) + factor(Z_CODE) 
  maz.re2 <- plm(form_2_maz_z, data = data_in_plm, model = "random")
  # summary(maz.re2)
  
  form_3_maz_z = MAIZEOPH_W ~ lag(data_in_plm$MAIZEOPH_W, 1)  + 
    A_Qnt   + A_AUC_Qnt   + MAIZEIMSEED +
    MAIZEDAMAGEAREA_P  +  I(MAIZEFERT_CHEMICAL_AMT/(MAIZEFERT_CHEMICAL_AREA+1)) +
    MAIZEEXTAREA + PPT_A_max + PPT_G_AUC_Qnt + PET_G_AUC_diff_mn   + 
      X_COORD + bs(elevation,3) + factor(Z_CODE) 
  maz.re3 <- plm(form_3_maz_z, data = data_in_plm, model = "random")
  # summary(maz.re3)
  
    form_4_maz_z = MAIZEOPH_W ~    lag(data_in_plm$MAIZEOPH_W, 1)+
    A_Qnt   + A_AUC_Qnt + PPT_A_max + PPT_G_AUC_Qnt + PET_G_AUC_diff_mn   + 
      X_COORD + bs(elevation,3) + factor(Z_CODE) 
  ...
  stargazer(maz.re1, maz.re2,  maz.re3, maz.re4,
          ...
          df=FALSE, digits=4,omit = "Z_CODE",omit.labels = 'Zone Dummy')
 ...
 # read data
  ...
  wht.re2 = WHEATOPH_W ~ lag(data_in_plm$WHEATOPH_W, 1)+ 
    ...
    WHEATAREA  + WHEATNIMSEED_P + WHEATDAMAGEAREA_P +    I(WHEATFERT_CHEMICAL_AMT / (WHEATFERT_CHEMICAL_AREA+1)) +    
