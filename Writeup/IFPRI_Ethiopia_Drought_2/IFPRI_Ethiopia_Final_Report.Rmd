...
maz_dam1_w_ea = factor(MAIZEDAMAGEAREA_P< 0.25 ) ~ A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  ...
  # save(maz.rf,file = './Outputs4Pred/maz.rf.MAIZEDAMAGEAREA_P_GT.25_zone_diff.RData')
  load('./Outputs4Pred/maz.rf.MAIZEDAMAGEAREA_P_GT.25_zone_diff.RData')
  ...
  cm_maz = confusionMatrix(data =  pp.test$pred, reference = (pp.test$MAIZEDAMAGEAREA_P<threshold), mode = "prec_recall")
  pp.train$pred_insp = predict(maz.rf,pp.train)
  cm_maz_insp = confusionMatrix(data =  pp.train$pred_insp, reference = (pp.train$MAIZEDAMAGEAREA_P<threshold), mode = "prec_recall")
...
```
`r thrd_imp_maz` tracks the effects of the average 10-day precipitation across the growing season. We can see that low levels of precipitation have the highest likelihood of crop damage. The likelihood of failure declines rapidly around a value of twenty, and reaches its lowest likelihood around 40mm. Past this we again see an increase the likelihood of damage, but at a slower rate. This later increase in the likelihood of damages likely points to the effects of excess precipitation or damaging storms.
...
#### Predict damages WHEAT ----------------------------------------------------
 ...
 # read data
  ...
  wht_dam1_w_ea =  factor(WHEATDAMAGEAREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  A_Qnt_diff+G_Qnt_diff+ PPT_G_AUC_Qnt_diff+PPT_G_mn_diff+ REGIONCODE+ 
  ...
  # print(wht.rf)
  #save(wht.rf,file = './Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P_GT_.25_zone_diff.RData')
  load('./Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P_GT_.25_zone_diff.RData')
  
 pp.test$pred = predict(wht.rf,pp.test)
 cm_wht =  confusionMatrix(data =  pp.test$pred, reference = (pp.test$WHEATDAMAGEAREA_P<threshold), mode = "prec_recall") 
  
 pp.train$pred_insp = predict(wht.rf,pp.train)
 cm_wht_insp = confusionMatrix(data =  pp.train$pred_insp, reference = (pp.train$WHEATDAMAGEAREA_P<threshold), mode = "prec_recall")
...
In the figure above, **we can see some interesting similarities and differences between the wheat and maize response to variability in greeness. Of particular interest is the broader range of variability that has no substantial influence on the likelihood of losses. This is not nescessarily pointing out that wheat is more resiliant to changes, but might be an indication of a broader range of natural variability in greeness that is expected for wheat. ** Similarly, we can look at the partial dependence plot for other variables of interest such as `r thrd_imp_wht`.
...
```{r Predict Damages Sorghum Regression, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
...
#### Predict damages Sorghum  ----------------------------------------------------
 ...
 # read data
  ...
  data_in = data_in[!is.na(data_in$SORGHUMDAMAGEAREA_P),]
   ...
   #set as < because caret reverses the outcome variable 
sor_dam1_w_ea = factor(SORGHUMDAMAGEAREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  A_Qnt_diff+G_Qnt_diff+ PPT_G_AUC_Qnt_diff+PPT_G_mn_diff+ REGIONCODE+ 
  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  #                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  # print(sor.rf)
  #save(sor.rf,file = './Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P_GT_.25_zone_diff.RData')
  load('./Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P_GT_.25_zone_diff.RData')
  
  pp.test$pred = predict(sor.rf, pp.test)
  cm_sor = confusionMatrix(data =  pp.test$pred, reference = (pp.test$SORGHUMDAMAGEAREA_P<threshold), mode = "prec_recall")
  pp.train$pred_insp = predict(sor.rf,pp.train)
  cm_sor_insp = confusionMatrix(data =  pp.train$pred_insp, reference = (pp.train$SORGHUMDAMAGEAREA_P<threshold), mode = "prec_recall")
...
bar_dam1_w_ea = factor(BARLEYDAMAGEAREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  A_Qnt_diff+G_Qnt_diff+ PPT_G_AUC_Qnt_diff+PPT_G_mn_diff+ REGIONCODE+ 
  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  #                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  # print(bar.rf)
  # save(bar.rf,file = './Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P_GT_.25_zone_diff.RData')
   load('./Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P_GT_.25_zone_diff.RData')
  ...
  cm_bar =  confusionMatrix(data =  pp.test$pred, reference = (pp.test$BARLEYDAMAGEAREA_P<threshold), mode = "prec_recall")
```
...
tef_dam1_w_ea = factor(TEFFDAMAGEAREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  A_Qnt_diff+G_Qnt_diff+ PPT_G_AUC_Qnt_diff+PPT_G_mn_diff+ REGIONCODE+ 
  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  #                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  # print(tef.rf)
  # save(tef.rf,file = './Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P_GT_.25_zone_diff.RData')
  load('./Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P_GT_.25_zone_diff.RData')
  ...
  cm_tef =   confusionMatrix(data =  pp.test$pred, reference = (pp.test$TEFFDAMAGEAREA_P<threshold), mode = "prec_recall")
  cm_tef
  0.29
```
...
```{r Predict Damages to all EAs, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
  ...
  #EAs_poly = readOGR("../Data/EnumerationAreas/EnumerationAreasSIN_Regions",'EnumerationAreasSIN_R_NAME_Addis Ababa') # use EA_code_merge as unique id
...
```{r All_Crops_Predict_Damages_to_all_EAs_by_year, echo=FALSE, fig.width=7, message=FALSE, warning=FALSE}
   setwd(Home_dir)
  EAs_point_DamageSum_allcrops_year = EAs_point %>% select(matches('DamageSum_allcrops_20')) %>% na.omit()
  names(EAs_point_DamageSum_allcrops_year)[names(EAs_point_DamageSum_allcrops_year)  %in% paste('DamageSum_allcrops',2009:2016 ,sep='_')] = 2009:2016
  EAs_point_DamageSum_allcrops_year =  EAs_point_DamageSum_allcrops_year  %>% gather('2009','2010','2011','2012','2013','2014','2015','2016',   key = "Year", value = "Failed_crop_count")
  
  # subset by year to speed up plotting
  set.seed(10)
  EAs_point_DamageSum_allcrops_year = EAs_point_DamageSum_allcrops_year%>% group_by(Year) %>% sample_n( 10000)

  # plot it 
  plot_dam_all_year = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data= EAs_point_DamageSum_allcrops_year ,  aes(color = Failed_crop_count ),size=0.25, alpha=0.5)+ facet_wrap( ~ Year, ncol=2) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 ) + scale_colour_gradientn(colours=rev(rainbow(4)))
                                      
  plot(plot_dam_all_year)

```

```{r All_Crops_Predict_Damages_to_all_EAs, echo=FALSE,  message=FALSE, warning=FALSE}
...
# subset to speed up plotting
  ...
  plot_dam_maz = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data= EAs_point_smp ,  aes(color = DamageSum_allcrops ),size=0.25, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 ) + scale_colour_gradientn(colours=rev(rainbow(4)))
                                      
  plot(plot_dam_maz)

```

Figure `r paste(Figure_number)`: paste('Maize: Predicted Damage > ',threshold,'%',sep='')`r Figure_number =Figure_number+1 `

```{r MAIZE_Predict_Damages_to_all_EAs, echo=FALSE,  message=FALSE, warning=FALSE}

  # MAIZE
  # subset for one crop and facet grid it
  setwd(Home_dir)
  EAs_point_maz = EAs_point %>% select(matches('maz')) %>% na.omit()
  names(EAs_point_maz)[names(EAs_point_maz)  %in% paste(2009:2016 ,'dam_maz',sep='_')] = 2009:2016
  EAs_point_maz =  EAs_point_maz  %>%
    gather('2009','2010','2011','2012','2013','2014','2015','2016',   key = "Year", value = "Successful_crop")
  # subset to speed up plotting
  set.seed(10)
  EAs_point_maz = EAs_point_maz%>% group_by(Year) %>% sample_n( 20000)

  # plot it 
  plot_dam_maz = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data=subset(EAs_point_maz, Successful_crop == F),  aes(color = Successful_crop ),size=0.25, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 )+ facet_wrap( ~ Year, ncol=4)+ theme(legend.position="none")
  xys = as.data.frame(st_coordinates(subset(EAs_point_maz, Successful_crop == F)))
  xys$Year = subset(EAs_point_maz, Successful_crop == F)$Year
  xys= xys[xys$Year>2012,]
 plot_dam_maz = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 )+geom_bin2d(data=xys,aes(X,Y),bins=20) + scale_fill_gradientn(colours=rev(rainbow(4)),na.value = 0 )  + facet_wrap( ~ Year, ncol=4)
  
  plot(plot_dam_maz)
  # ggsave(plot = plot_dam_maz,'./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_maz.pdf')
```

Figure `r paste(Figure_number)`: paste('Barley: Predicted Damage > ',threshold,'%',sep='')`r Figure_number =Figure_number+1 `

```{r BARLEY_Predict_Damages_to_all_EAs, echo=FALSE, message=FALSE, warning=FALSE}
  # BARLEY
  # subset for one crop and facet grid it
  EAs_point_bar = EAs_point %>% select(matches('bar')) %>% na.omit()
  names(EAs_point_bar)[names(EAs_point_bar)  %in% paste(  2009:2016 ,'dam_bar',sep='_')] = 2009:2016
  EAs_point_bar =  EAs_point_bar  %>%
    gather('2009','2010','2011','2012','2013','2014','2015','2016',   key = "Year", value = "Successful_crop")
  # subset to speed up plotting
  set.seed(10)
  EAs_point_maz = EAs_point_maz%>% group_by(Year) %>% sample_n( 20000)
  ...
  # plot it 
  plot_dam_bar = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data=subset(EAs_point_bar, Successful_crop == F),  aes(color = Successful_crop ),size=0.25, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 )+ facet_wrap( ~ Year, ncol=4)+ theme(legend.position="none")
  
  plot(plot_dam_bar)
  # ggsave(plot = plot_dam_bar,'./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_bar.pdf')
```

Figure `r paste(Figure_number)`: paste('Wheat: Predicted Damage > ',threshold,'%',sep='')`r Figure_number =Figure_number+1 `

```{r WHEAT_Predict_Damages_to_all_EAs, echo=FALSE,  message=FALSE, warning=FALSE}
  
  # WHEAT 
  # subset for one crop and facet grid it
  EAs_point_wht = EAs_point %>% select(matches('wht')) %>% na.omit()
  names(EAs_point_wht)[names(EAs_point_wht)  %in% paste(2009:2016 ,'dam_wht',sep='_')] = 2009:2016
  EAs_point_wht =  EAs_point_wht  %>%
    gather('2009','2010','2011','2012','2013','2014','2015','2016',   key = "Year", value = "Successful_crop")
  # subset to speed up plotting
  set.seed(10)
  EAs_point_maz = EAs_point_maz%>% group_by(Year) %>% sample_n( 20000)
  ...
  # plot it 
  plot_dam_wht = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data=subset(EAs_point_wht, Successful_crop == F),  aes(color = Successful_crop ),size=0.25, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 )+ facet_wrap( ~ Year, ncol=4)+  theme(legend.position="none")
                          
  plot(plot_dam_wht)
  # ggsave(plot = plot_dam_wht,'./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_wht.pdf')
```

Figure `r paste(Figure_number)`: paste('Sorghum: Predicted Damage > ',threshold,'%',sep='')`r Figure_number =Figure_number+1 `

```{r SORGHUM_Predict_Damages_to_all_EAs, echo=FALSE,  message=FALSE, warning=FALSE}

  # SORGHUM 
  # subset for one crop and facet grid it
  EAs_point_sor = EAs_point %>% select(matches('sor')) %>% na.omit()
  names(EAs_point_sor)[names(EAs_point_sor)  %in% paste(2009:2016 ,'dam_sor',sep='_')] = 2009:2016
  EAs_point_sor =  EAs_point_sor  %>%
    gather('2009','2010','2011','2012','2013','2014','2015','2016',   key = "Year", value = "Successful_crop")
  # subset to speed up plotting
  set.seed(10)
  EAs_point_maz = EAs_point_maz%>% group_by(Year) %>% sample_n( 20000)
  ...
  # plot it 
  plot_dam_sor = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data=subset(EAs_point_sor, Successful_crop == F),  aes(color = Successful_crop ),size=0.25, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 )+ facet_wrap( ~ Year, ncol=4)+  theme(legend.position="none")
  
  plot(plot_dam_sor)
  # ggsave(plot = plot_dam_sor,'./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_sor.pdf')
```

Figure `r paste(Figure_number)`: paste('Teff: Predicted Damage > ',threshold,'%',sep='')`r Figure_number =Figure_number+1 `

```{r TEFF_Predict_Damages_to_all_EAs, echo=FALSE, message=FALSE, warning=FALSE}
  
  # TEFF 
  # subset for one crop and facet grid it
  EAs_point_tef = EAs_point %>% select(matches('tef')) %>% na.omit()
  names(EAs_point_tef)[names(EAs_point_tef)  %in% paste(2009:2016 ,'dam_tef',sep='_')] = 2009:2016
  EAs_point_tef =  EAs_point_tef  %>%
    gather('2009','2010','2011','2012','2013','2014','2015','2016',   key = "Year", value = "Successful_crop")
  # subset to speed up plotting
  set.seed(10)
  EAs_point_maz = EAs_point_maz%>% group_by(Year) %>% sample_n( 20000)
  ...
  # plot it 
  plot_dam_tef = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +  
    geom_sf(data=subset(EAs_point_tef, Successful_crop == F),  aes(color = Successful_crop ),size=0.25, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 )+ facet_wrap( ~ Year, ncol=4)+  theme(legend.position="none")
  
  plot(plot_dam_tef)
  # ggsave(plot = plot_dam_tef,'./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_tef.pdf')
