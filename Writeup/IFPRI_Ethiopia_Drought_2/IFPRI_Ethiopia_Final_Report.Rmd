...
#### Predict damages MAIZE ----------------------------------------------------
 ...
 maz_dam1_w_ea = factor(MAIZEDAMAGEAREA_P<0.3 ) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  #                 prox=T,allowParallel=T,tuneGrid = expand.grid(mtry=seq(5,17,1)))
  ...
  # load('./Outputs4Pred/maz.rf.MAIZEDAMAGEAREA_P_GT.3_zone.RData')
...
#### Predict damages WHEAT ----------------------------------------------------
 ...
 # read data
  ...
  load('./Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P_GT.3_zone_v2.RData')
...
sor_dam1_w_ea = factor(SORGHUMDAMAGEAREA_P<0.3) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  # Impute values   THIS ALSO CENTERS DATA 
  set.seed(123)
  ...
  load('./Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P_GT.3_zone_v2.RData')
 ...
 cm_sor = confusionMatrix(data =  pp.test$pred, reference = (pp.test$SORGHUMDAMAGEAREA_P<0.3), mode = "prec_recall")
```

### Sorghum Damages
The confusion matrix compares predicted values against the observed (reference). We also report the a) overall accuracy, 2) Kappa coefficient, and 3) the recall rate[^3] as seen in table `r paste(Table_number)` below:

*Table `r paste(Table_number)`: Confusion matrix for reported damages > 30% for Sorghum* `r  Table_number=Table_number+1`
`r library(pander); pander(cm_sor$table, caption = "FALSE = Crop damages greater than 30%")`
*Table `r paste(Table_number)`: Performance metrics for reported damages > 30% for Sorghum* `r  Table_number=Table_number+1`
...
bar_dam1_w_ea = factor(BARLEYDAMAGEAREA_P<0.3) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  load('./Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P_GT.3_zone_v2.RData')
...
cm_bar =  confusionMatrix(data =  pp.test$pred, reference = (pp.test$BARLEYDAMAGEAREA_P<0.3), mode = "prec_recall")
```
   
### Barley Damages
The confusion matrix compares predicted values against the observed (reference). We also report the a) overall accuracy, 2) Kappa coefficient, and 3) the recall rate[^3] as seen in table `r paste(Table_number)` below:

*Table `r paste(Table_number)`: Confusion matrix for reported damages > 30% for barley* `r  Table_number=Table_number+1`
`r library(pander); pander(cm_bar$table, caption = "FALSE = Crop damages greater than 30%")`
*Table `r paste(Table_number)`: Performance metrics for reported damages > 30% for barley* `r  Table_number=Table_number+1`
...
tef_dam1_w_ea = factor(TEFFDAMAGEAREA_P<0.3) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  cm_tef =   confusionMatrix(data =  pp.test$pred, reference = (pp.test$TEFFDAMAGEAREA_P<0.3), mode = "prec_recall")
```
...
*Table `r paste(Table_number)`: Confusion matrix for reported damages > 30% for barley* `r  Table_number=Table_number+1`
`r library(pander); pander(cm_tef$table, caption = "FALSE = Crop damages greater than 30%")`
*Table `r paste(Table_number)`: Performance metrics for reported damages > 30% for barley* `r  Table_number=Table_number+1`
...
```{r Predict Damages to all EAs,  echo=FALSE, message=FALSE, warning=FALSE,fig.height=7, include=FALSE} 
  ...
  # before prediction, scale and center 
  all_eas_data = predict(preProcess(all_eas_data, method = c("center", "scale")),all_eas_data)

  # load & predict damage model
  load('./Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P_GT.3_zone.RData')
  ...
  all_eas_dam_sor =  spread(unique(all_eas_data[,list(Year,RK_NAME,RK_CODE,R_NAME,R_CODE,Z_NAME,Z_CODE,W_NAME,
     W_CODE,EA_NAME,EA_merge,pred_dam_sor) ]), key = Year, value = pred_dam_sor)
  names(all_eas_dam_sor)[names(all_eas_dam_sor)  %in% 2009:2016] =
     paste(names(all_eas_dam_sor)[names(all_eas_dam_sor)  %in% 2009:2016],'dam_sor',sep='_')
  all_eas_dam_bar =  spread(unique(all_eas_data[,list(Year,RK_NAME,RK_CODE,R_NAME,R_CODE,Z_NAME,Z_CODE,W_NAME,
     W_CODE,EA_NAME,EA_merge,pred_dam_bar) ]), key = Year, value = pred_dam_bar)
  ...
  all_eas_dam_wht =  spread(unique(all_eas_data[,list(Year,RK_NAME,RK_CODE,R_NAME,R_CODE,Z_NAME,Z_CODE,W_NAME,
      W_CODE,EA_NAME,EA_merge,pred_dam_wht) ]), key = Year, value = pred_dam_wht)
  ...
  # remove duplicates from Maize and Teff
  ...
  jnd.tbl = all_eas_dam_tef %>% select(matches('20|EA_merge')) %>% unique()%>% left_join( jnd.tbl, . , by ='EA_merge')
  EAs_poly = left_join(EAs_poly,jnd.tbl,by='EA_merge')
  #saveRDS(EAs_poly,'./Outputs4Pred/all_eas_damage_V4.rds')
  ...
  EAs_poly = readRDS('./Outputs4Pred/all_eas_damage_V4.rds')  
 
  # check number of damaged EAs 
  EAs_poly  %>% as.data.frame() %>% select(matches('tef')) %>% apply(.,2,function(x) as.numeric(as.character(x))) %>% colSums( na.rm = T)
    
    
  
  # import joined damage data 
  EAs_poly = readRDS("./Outputs4Pred/all_eas_damage_V4.rds")
  
  # plot results 
  ggplot(EAs_poly) +  geom_sf(aes(fill = '2009_dam_tef',colour=NULL))
