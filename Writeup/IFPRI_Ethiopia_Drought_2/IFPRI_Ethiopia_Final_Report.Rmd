...
Figure `r paste(Figure_number)`: Median rainfall per 10 day period 2009/10-2015/16 compared to 2014/15 and 2015/16`r Figure_number =Figure_number+1 `
...
Before we run any models we break the data into two groups, with 85% of all sub-kebele units being retained for the training/tuning/cross-validation of the models, and 15% of these held aside to provide independent measures of out-of-sample performance. 
...
```{r Predict Damages Maize Regression_by_region, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
#### Predict damages MAIZE ----------------------------------------------------
  # read data
for(region_iter in c(1,3,4,7)){
    setwd(Home_dir) 
    ######## MAIZE   
    # region code  1= Tigray 3=amhara  4=oromia 7 = SNNP
    ...
    data_in = read.dta13(paste("./Outputs4Pred/AgSS_2010_15_Compiled_panel_merged_clean_v4.dta",sep=''))
    data_in = data_in[,!(names(data_in) %in% c("_merge") ) ]
    data_in = data_in[data_in$REGIONCODE==region_iter,  ]
    name_lookup = data.table(region_id = c(1,3,4,7),region_name=c('Tigray','Amhara','Oromia','SNNP'))
    region_name = name_lookup[region_id ==region_iter,]$region_name
    ...
    # assign to cm_maz_region_name
    assign(x = paste('cm_maz',region_name,sep='_'),value = confusionMatrix(data =  pp.test$pred, reference =   (pp.test$MAIZEDAMAGE_DROUGHT_AREA_P<threshold), mode = "prec_recall"))
 ...
 *Table `r paste(Table_number)`: Region specific model performance metrics for reported damages > `r paste(threshold,'%',sep='')` for Maize* `r  Table_number=Table_number+1`
```{r Predict Damages Maize results by region, echo=FALSE, message=FALSE, warning=FALSE}
maz_reg = data.frame( 
Accuracy=c(round(cm_maz_Amhara$overall['Accuracy'],2),round(cm_maz_Oromia$overall['Accuracy'],2),round(cm_maz_SNNP$overall['Accuracy'],2),round(cm_maz_Tigray$overall['Accuracy'],2)    ), Kappa=c(round(cm_maz_Amhara$overall['Kappa'],2),round(cm_maz_Oromia$overall['Kappa'],2),round(cm_maz_SNNP$overall['Kappa'],2),round(cm_maz_Tigray$overall['Kappa'],2)    ), 
Recall=c(round(cm_maz_Amhara$byClass['Recall'],2),round(cm_maz_Oromia$byClass['Recall'],2),round(cm_maz_SNNP$byClass['Recall'],2),round(cm_maz_Tigray$byClass['Recall'],2)    ),
row.names =c('Amhara','Oromia','SNNP','Tirgray'))
pander(maz_reg)
```

Looking at models trained and tested independently by region we can see that model accuracy varies substantially by region. We see maximum overall accuracy of `r max(maz_reg$Accuracy)*100`%, and a maximum recall rate (the percent of substantial losses correctly identified) of `r max(maz_reg$Recall)*100`% for `r row.names(maz_reg[maz_reg$Recall==max(maz_reg$Recall),])`. This variation in performance by region is likely due to difficulties in field delination with a 250m pixel, and problems with land cover identification. These problems could likely be alleviated by moving toward higher resolutions data products, like the 30m Harmonized Landsat Sentinel data (HLS) once available. 
...
Figure `r paste(Figure_number)`: Variable importance plot for maize damage prediction`r maz_imp_fig=Figure_number; Figure_number =Figure_number+1 `
...
thrd_imp_maz = varImp_maz$importance$name[10]
...
```
`r secd_imp_maz` is a measure of variability in NDVI across the entire year (including both the Meher and Belg seasons). We see a near linear decrease in the likelihood of damage starting from a value of around 600, with a minima near 1750. The high likelihood of failure corresponding with very low values of NDVI variance likely relates to early season trouble including a failure to germinate, or early damage leading to crop failure. These early catastrophic losses would have little change in NDVI (greeness) across the year and therefore very low values of `r secd_imp_maz`. The minima in the likelihood of a loss occures at a relatively high level of variance (around the 3rd quintile), this is likely related to the fact that a plot of healthy plants that is harvested and tilled, will have relatively high variance (especially in dual-cropping Meher and Belg areas). We can see values around 2000 that extremely high variance again corresponds to increased likelihood of losses, as dramatic shifts in plant health would likely correspond to the highest levels of variance. We can see how variance in NDVI corresponds to crop failures with variables such as `r thrd_imp_maz`.
...
```
`r thrd_imp_maz` tracks the effects of the average 10-day precipitation across the growing season. We can see that low levels of precipitation have the highest likelihood of crop damage. The likelihood of failure declines rapidly around a value of 20mm, and reaches its lowest likelihood around 40mm. Past this, we again see an increase the likelihood of damage, but at a slower rate. This later increase in the likelihood of damages likely points to the effects of excess precipitation or damaging storms.
...
```{r Predict Damages Wheat Regression region loop, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
#### Predict damages WHEAT ----------------------------------------------------
for(region_iter in c(1,3,4,7)){  # read data
    setwd(Home_dir) 
   ...
   pp.test$pred = predict(wht.rf,pp.test)
    # assign to cm_maz_region_name
    assign(x = paste('cm_wht',region_name,sep='_'),value = confusionMatrix(data =  pp.test$pred, reference =   (pp.test$WHEATDAMAGE_DROUGHT_AREA_P<threshold), mode = "prec_recall"))    
}
...
*Table `r paste(Table_number)`: Region specific model performance metrics for reported damages > `r paste(threshold,'%',sep='')` for Wheat* `r  Table_number=Table_number+1`
```{r Predict Damages Wheat results by region, echo=FALSE, message=FALSE, warning=FALSE}
wht_reg = data.frame( 
Accuracy=c(round(cm_wht_Amhara$overall['Accuracy'],2),round(cm_wht_Oromia$overall['Accuracy'],2),round(cm_wht_SNNP$overall['Accuracy'],2),round(cm_wht_Tigray$overall['Accuracy'],2)    ), Kappa=c(round(cm_wht_Amhara$overall['Kappa'],2),round(cm_wht_Oromia$overall['Kappa'],2),round(cm_wht_SNNP$overall['Kappa'],2),round(cm_wht_Tigray$overall['Kappa'],2)    ), 
Recall=c(round(cm_wht_Amhara$byClass['Recall'],2),round(cm_wht_Oromia$byClass['Recall'],2),round(cm_wht_SNNP$byClass['Recall'],2),round(cm_wht_Tigray$byClass['Recall'],2)    ),
row.names =c('Amhara','Oromia','SNNP','Tirgray'))
pander(wht_reg)
```

Looking at models trained and tested independently by region we can see that model accuracy varies substantially by region. We see maximum overall accuracy of `r max(wht_reg$Accuracy)*100`% for `r row.names(wht_reg[wht_reg$Accuracy==max(wht_reg$Accuracy),])`, and a maximum recall rate (% of substantial losses correctly identified) of `r max(wht_reg$Recall)*100`% for `r row.names(wht_reg[wht_reg$Recall==max(wht_reg$Recall),])`. This variation in performance by region is likely due to difficulties in field delination with a 250m pixel, and problems with land cover identification in an extremely heterogenous environment.
 
We can also examine the relative importance of each variable to the national-level model, as well as its estimated non-linear relationship with the dependent variable through partial dependency plots. First, let's examine the relative importance of each variable in the random forest through the role it played in Gini coefficient loss.

Figure `r paste(Figure_number)`: Variable importance plot for maize damage prediction`r wht_imp_fig=Figure_number; Figure_number =Figure_number+1`
...
From the figure above we can see that `r most_imp_wht` has the highest relative importance in the random forest model. `r most_imp_wht` is the first difference of G_Qnt (G_Qnt_diff), where G_Qnt is the 90th percentile of NDVI values in the Meher season. As such the high NDVI values  reflect a measure of plant health, with higher `r  rownames(varImp_wht$importance)[which.max(varImp_wht$importance$Overall)]` values corresponding to strong plant health (higher leaf area, low water stress) at some point in the growing season. We can also see that the most important variables are largely comprised of variables summarizing NDVI, with the exception of PPT_G_mn which is the mean value of precipitation for each 10 day period across the growing season.  

Interestingly, comparing figures `r wht_imp_fig` and `r maz_imp_fig`, we see that measures of variability such as A_sd and G_sd are very  important for Maize but not for wheat. This might reflect physilogical differences between plants, where variability in leaf color more greatly effects yields for Maize. These differences between models indicate a need to run plant specific models. 

We can now look at the partial dependence plots to help understand the non-linear response of reported damages to `r most_imp_wht`.  
...
```
In the figure above, we can see with `r most_imp_wht`, similar to maize, how the likelihood of reporting maize losses decreases rapidly after -1000, with a minima near zero. The likelihood of losses increase at the upper end of the distribution. These losses at upper end of the distribution might reflect poor management such as failure to control weeds, or excessively narrow row spacing. This consistency across independent crop models is a strong indication that these models are picking up on meaningful non-linear relations. Similarly, we can look at the partial dependence plot for other variables of interest such as `r secd_imp_wht`.

Figure `r paste(Figure_number)`: Partial dependency plot for influence of `r secd_imp_wht` on wheat damage prediction`r Figure_number =Figure_number+1 `
...
Looking again at Figure `r wht_imp_fig`, we can see that the most important variables for Wheat are largely comprised of measures of the upper level of the distribution (G_mx, G_qnt, A_mx). This points to one of the advantages of machine learning, since these algorithms can exploit even minor difference between otherwise colinear variables in order to provide a reliable model fit. Looking at one (G_mx) we can see a familiar non-linear pattern with declining likelihood of losses up to some point and increases thereafter. Extremely low levels of G_mx likely correspond to failure in germination, low planting density, etc and extremely high levels of G_mx likely corresponds to a failure to weed or an excess of rain. Similarly, we can look at the partial dependence plot for other variables of interest such as `r thrd_imp_wht`.
...
```
In the figure above, we can see some interesting similarities and differences between the wheat and maize response to mean precipitation over the growing season. Of particular interest is in areas greater than 40mm. The likelihood that substantial losses are reported doesn't increase meaningfully until over 60mm. In comparison, maize losses increased rapidly past 40mm. Here the likelihood of lossess does not increase rapidly until 60mm. Althogh these damages would likely be linked to losses from rust or mold, or physical damage from storms all of which increase with excess rainfall. The differences between maize and wheat however demonstrate the resiliance of wheat to changes in precipitation.
...
```{r Predict Damages Sorghum Regression by region, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
#### Predict damages Sorghum  ----------------------------------------------------
for(region_iter in c(1,3,4,7)){  
  setwd(Home_dir) 
...
sor_dam1_w_ea = factor(SORGHUMDAMAGE_DROUGHT_AREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  ...
  pp.test$pred = predict(sor.rf, pp.test)
  # assign to cm_maz_region_name
    assign(x = paste('cm_sor',region_name,sep='_'),value = confusionMatrix(data =  pp.test$pred, reference =   (pp.test$SORGHUMDAMAGE_DROUGHT_AREA_P<threshold), mode = "prec_recall")) 
 }
```
 ...
 *Table `r paste(Table_number)`: Region specific model performance metrics for reported damages > `r paste(threshold,'%',sep='')` for Sorghum* `r  Table_number=Table_number+1`
```{r Predict Damages Sorghum results by region, echo=FALSE, message=FALSE, warning=FALSE}
sor_reg = data.frame( 
Accuracy=c(round(cm_sor_Amhara$overall['Accuracy'],2),round(cm_sor_Oromia$overall['Accuracy'],2),round(cm_sor_SNNP$overall['Accuracy'],2),round(cm_sor_Tigray$overall['Accuracy'],2)    ), Kappa=c(round(cm_sor_Amhara$overall['Kappa'],2),round(cm_sor_Oromia$overall['Kappa'],2),round(cm_sor_SNNP$overall['Kappa'],2),round(cm_sor_Tigray$overall['Kappa'],2)    ), 
Recall=c(round(cm_sor_Amhara$byClass['Recall'],2),round(cm_sor_Oromia$byClass['Recall'],2),round(cm_sor_SNNP$byClass['Recall'],2),round(cm_sor_Tigray$byClass['Recall'],2)    ),
row.names =c('Amhara','Oromia','SNNP','Tirgray'))
pander(sor_reg)
```

Looking at models trained and tested independently by region we can see that model accuracy varies substantially by region. We see maximum overall accuracy of `r max(sor_reg$Accuracy)*100`% for `r row.names(sor_reg[sor_reg$Accuracy==max(sor_reg$Accuracy),])`, and a maximum recall rate (% of substantial losses correctly identified) of `r max(sor_reg$Recall)*100`% for `r row.names(sor_reg[sor_reg$Recall==max(sor_reg$Recall),])`. This variation in performance by region is likely due to difficulties in field delination with a 250m pixel, and problems with land cover identification.
...
```{r Predict Damages Barley Regression by Region, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
#### Predict damages BARLEY  ----------------------------------------------------
for(region_iter in c(1,3,4,7)){  
  setwd(Home_dir) 
...
bar_dam1_w_ea = factor(BARLEYDAMAGE_DROUGHT_AREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  ...
  pp.test$pred = predict(bar.rf, pp.test)
    # assign to cm_maz_region_name
    assign(x = paste('cm_bar',region_name,sep='_'),value = confusionMatrix(data =  pp.test$pred, reference =   (pp.test$BARLEYDAMAGE_DROUGHT_AREA_P<threshold), mode = "prec_recall"))   
  }
```   
...
*Table `r paste(Table_number)`: Region specific model performance metrics for reported damages > `r paste(threshold,'%',sep='')` for Barley* `r  Table_number=Table_number+1`
```{r Damages Wheat results by region, echo=FALSE, message=FALSE, warning=FALSE}
  bar_reg = data.frame( 
  Accuracy=c(round(cm_bar_Amhara$overall['Accuracy'],2),round(cm_bar_Oromia$overall['Accuracy'],2),round(cm_bar_SNNP$overall['Accuracy'],2),round(cm_bar_Tigray$overall['Accuracy'],2)    ), Kappa=c(round(cm_bar_Amhara$overall['Kappa'],2),round(cm_bar_Oromia$overall['Kappa'],2),round(cm_bar_SNNP$overall['Kappa'],2),round(cm_bar_Tigray$overall['Kappa'],2)    ), 
  Recall=c(round(cm_bar_Amhara$byClass['Recall'],2),round(cm_bar_Oromia$byClass['Recall'],2),round(cm_bar_SNNP$byClass['Recall'],2),round(cm_bar_Tigray$byClass['Recall'],2)    ),
  row.names =c('Amhara','Oromia','SNNP','Tirgray'))
  pander(bar_reg)
```

Looking at models trained and tested independently by region we can see that model accuracy varies substantially by region. We see maximum overall accuracy of `r max(bar_reg$Accuracy)*100`% for `r row.names(bar_reg[maz_reg$Accuracy==max(bar_reg$Accuracy),])`, and a maximum recall rate (% of substantial losses correctly identified) of `r max(bar_reg$Recall)*100`% for `r row.names(bar_reg[bar_reg$Recall==max(bar_reg$Recall),])`. This variation in performance by region is likely due to difficulties in field delination with a 250m pixel, and problems with land cover identification.   
...
```{r Predict Damages Teff Regression by region, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
#### Predict damages TEFF  ----------------------------------------------------
for(region_iter in c(1,3,4,7)){  # read data
  setwd(Home_dir) 
...
tef_dam1_w_ea = factor(TEFFDAMAGE_DROUGHT_AREA_P<0.25) ~  A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  ...
  pp.test$pred = predict(tef.rf, pp.test)
  # assign to cm_maz_region_name
  assign(x = paste('cm_tef',region_name,sep='_'),value = confusionMatrix(data =  pp.test$pred, reference =   (pp.test$TEFFDAMAGE_DROUGHT_AREA_P<threshold), mode = "prec_recall"))      
  }
```
...
*Table `r paste(Table_number)`: Region specific model performance metrics for reported damages > `r paste(threshold,'%',sep='')` for Teff* `r  Table_number=Table_number+1`
```{r Damages Wheat results by region table, echo=FALSE, message=FALSE, warning=FALSE}
tef_reg = data.frame( 
Accuracy=c(round(cm_tef_Amhara$overall['Accuracy'],2),round(cm_tef_Oromia$overall['Accuracy'],2),round(cm_tef_SNNP$overall['Accuracy'],2),round(cm_tef_Tigray$overall['Accuracy'],2)    ), Kappa=c(round(cm_tef_Amhara$overall['Kappa'],2),round(cm_tef_Oromia$overall['Kappa'],2),round(cm_tef_SNNP$overall['Kappa'],2),round(cm_tef_Tigray$overall['Kappa'],2)    ), 
Recall=c(round(cm_tef_Amhara$byClass['Recall'],2),round(cm_tef_Oromia$byClass['Recall'],2),round(cm_tef_SNNP$byClass['Recall'],2),round(cm_tef_Tigray$byClass['Recall'],2)    ),
row.names =c('Amhara','Oromia','SNNP','Tirgray'))
pander(tef_reg)
```

Looking at models trained and tested independently by region we can see that model accuracy varies substantially by region. We see maximum overall accuracy of `r max(tef_reg$Accuracy)*100`% for `r row.names(tef_reg[tef_reg$Accuracy==max(tef_reg$Accuracy),])`, and a maximum recall rate (% of substantial losses correctly identified) of `r max(tef_reg$Recall)*100`% for `r row.names(wht_reg[tef_reg$Recall==max(tef_reg$Recall),])`. This variation in performance by region is likely due to difficulties in field delination with a 250m pixel, and problems with land cover identification.   

## Predict damages out of sample 
One possible use case of these models, once improved through higher resolution, better agricultural land use maps, is to predict losses outside of AgSS sample areas. This will allow policy makers to identify potential at-risk communities rapidly at low cost. Given the importance of maximum and 90th percentile values of NDVI in model performance, a reasonable estimate could be made mid-season (or just past peak greeness). As example of how this might work we extracted NDVI statistics for all 75k+ sub-kebeles and used the model trained on AgSS sample areas. 
...
# 
#   table(all_eas_data$pred_dam_sor, exclude = NULL)
# 
...
#   all_eas_dam_sor$pred_dam_sor = all_eas_dam_sor$pred_dam_sor == T  # convert from factor to logical
...
#   all_eas_dam_bar$pred_dam_bar = all_eas_dam_bar$pred_dam_bar == T
...
#   all_eas_dam_wht$pred_dam_wht = all_eas_dam_wht$pred_dam_wht == T
...
#   all_eas_dam_maz_sub$pred_dam_maz = all_eas_dam_maz_sub$pred_dam_maz == T
...
#   all_eas_dam_tef_sub$pred_dam_tef = all_eas_dam_tef_sub$pred_dam_tef == T
...
# 
#   # check number of damaged EAs ( Damaged is == FALSE)
#   EAs_poly  %>% as.data.frame() %>% select(matches('bar')) %>% apply(.,2,function(x) x==F ) %>% colSums( na.rm = T)
...
Figure `r paste(Figure_number)`: Count of Predicted Damages (>25%) at the Sub-Kebele Level 2009-2016   `r Figure_number =Figure_number+1 `
...
```{r All_Crops_Predict_Damages_to_all_EAs, echo=FALSE, fig.width=7, message=FALSE, warning=FALSE}
  ...
  set.seed(10)
  EAs_point_smp = sample_n(EAs_point,50000)
  
  # plot it 
   plot_dam_maz = ggplot()+  geom_sf(data=Reg_poly,  aes( ),fill = 'grey20',size=0.25 ) +
    geom_sf(data= EAs_point_smp[EAs_point_smp$R_CODE %in% c(1,3,4,7),] ,  aes(color = DamageSum_allcrops ),size=0.5, alpha=0.5) +  geom_sf(data=Reg_poly,  aes( ),colour='grey80',fill = NA,size=0.25 ) + scale_colour_gradientn(colours=rev(rainbow(4)),name='Damage Count')

   plot(plot_dam_maz)

```

In this particular example, we provide an estimate of the total number of losses (for all crops) for the 2010/11-2015/16 growing season. As you can see maps like this could be used to identify isolated clusters of at-risk communities. Or if provided midway through the growing season could act as a method for rapid assessment to help target food aid or agricultural extension.    
