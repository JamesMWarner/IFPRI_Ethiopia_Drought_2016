...
#### Predict damages MAIZE ----------------------------------------------------
 ...
 # read data
  ...
  data_in$G_height_diff = as.numeric(lag(data_in_plm$G_mx-data_in_plm$G_min,1))
  data_in$A_height =      data_in_plm$A_max-data_in_plm$A_min
  data_in$A_height_diff = as.numeric(lag(data_in_plm$A_max-data_in_plm$A_min,1))
  data_in$G_max_diff =    as.numeric(lag(data_in_plm$G_mx,1))
  data_in$A_max_diff  =   as.numeric(lag(data_in_plm$A_max,1))
  data_in$A_Qnt_diff =    as.numeric(lag(data_in_plm$A_Qnt,1))
  data_in$G_Qnt_diff  =   as.numeric(lag(data_in_plm$G_Qnt,1))
  data_in$PPT_G_AUC_Qnt_diff = as.numeric(lag(data_in_plm$PPT_G_AUC_Qnt,1))
  data_in$PPT_G_mn_diff = as.numeric(lag(data_in_plm$PPT_G_mn,1))
  rm(data_in_plm)
...
maz_dam1_w_ea = factor(MAIZEDAMAGEAREA_P< 0.25 ) ~A_height+A_height_diff+G_height+G_height_diff+ G_max_diff+A_max_diff+
  A_Qnt_diff+G_Qnt_diff+ REGIONCODE+
  PPT_G_AUC_Qnt_diff+PPT_G_mn_diff+A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + 
  ...
  #https://analyticsdataexploration.com/xgboost-model-tuning-in-crossvalidation-using-caret-in-r/
   ...
   # maz.xg<-train(maz_dam1_w_ea,data=pp.train,method="xgbTree",   metric="Kappa",
   #                trControl=trainControl( index = groups,method = "cv",number = 3, allowParallel = T),  #
   #                prox=T, tuneGrid =  expand.grid(nrounds = 75, max_depth = c(4, 6,  10),
   #                eta = c(0.01), gamma = c(0), colsample_bytree = c(0.5, 0.75, 1.0),  # eta learning rate (0.01,0.001)
   #                min_child_weight = c(1, 2),subsample=c(1) ))

  #  maz.xg<-train(maz_dam1_w_ea,data=pp.train,method="xgbTree",metric="Recall",objective = "binary:logistic",stratify=T,
  #                 trControl=trainControl( index = groups,method = "cv",number = 3,allowParallel=T),   
  #                 prox=T, tuneGrid =  expand.grid(nrounds = 75, max_depth = c(6),
  #                 eta = c(0.2), gamma = c(0), colsample_bytree = c( 1.0),  # eta learning rate (0.01,0.001)
  #                 min_child_weight = c(1),subsample=c(1) ))
  # save(maz.xg,file = './Outputs4Pred/maz.xg.MAIZEDAMAGEAREA_P_GT_.2_zone.RData')
  ...
  cm_maz
  # results summary 
  #cutoff 0.50, metric="Kappa", accuracy 0.89, recall 0.18  
  #cutoff 0.30, metric="Kappa", accuracy 0.66, recall 0.24  
  #cutoff 0.25, metric="Kappa", accuracy 0.72, recall 0.34  
  #cutoff 0.2, metric="Kappa", accuracy 0.669, recall 0.43  
  #cutoff 0.2, metric="Recall", accuracy 0.669, recall 0.43  
  #cutoff 0.1, metric="Kappa", accuracy 0.646, recall 0.7798  

```


### Maize Damages
We can evaluate accuracy with a number of metrics on our independent testing data set described in the "Testing Data: Out-of-sample Performance" section above. The first, the confusion matrix compares predicted values against the observed (reference). We also report the a) overall accuracy, 2) Kappa coefficient, and 3) the recall rate[^3] as seen in table `r paste(Table_number)` below:

*Table `r paste(Table_number)`: Confusion matrix for reported damages > `r paste(threshold,'%',sep='')` for Maize*   
...
We see reasonable identification of reported maize crop losses with a recall rate of `r round(cm_maz$byClass['Recall'],2)`, indicating that `r round(cm_maz$byClass['Recall'],2)*100`% of all substantial loss cases were predicted in our completely independent *testing* data. In all, `r cm_maz$table['FALSE','FALSE']` out of `r cm_maz$table['FALSE','FALSE']+ cm_maz$table['TRUE','FALSE']` reported cases were predicted, and only `r cm_maz$table['FALSE','TRUE']` cases were falsely predicted as crop failure areas. 
...
dotchart(top_ten[,'Overall'],labels=top_ten[,'name'],cex=.7,xlab="Importance")
...
#### Predict damages WHEAT ----------------------------------------------------
 ...
 # read data
  ...
  wht_dam1_w_ea =  factor(WHEATDAMAGEAREA_P<0.2) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  # print(wht.rf)
  # save(wht.rf,file = './Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P_GT_.2_zone_v2.RData')
  load('./Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P_GT_.2_zone_v2.RData')
...
For the case of wheat, we see impressive identification of reported wheat crop losses with a recall rate of `r round(cm_wht$byClass['Recall'],2)`, indicating that `r round(cm_wht$byClass['Recall'],2)*100`% of all substantial loss cases were predicted in our completely independent *testing* data. In all, `r cm_wht$table['FALSE','FALSE']` out of `r cm_wht$table['FALSE','FALSE']+ cm_wht$table['TRUE','FALSE']` reported cases were predicted, and only `r cm_wht$table['FALSE','TRUE']` cases were falsely predicted as crop failure areas. 
...
thrd_imp_wht = varImp_wht$importance$name[5]
...
```
In the figure above, **we can see, similar to maize, how the likelihood of reporting maize losses decreases rapidly after one standard deviation below the mean value of `r most_imp_wht`, with the likelihood of losses increasing at the upper end of the distribution. These losses at upper end of the distribution might reflect excess rainfall, or late season losses from lodging or frost. Ideally additional variables would capture the effects of these late season dynamics in ways that maximum NDVI cannot.** Similarly, we can look at the partial dependence plot for other variables of interest such as `r secd_imp_wht`.
...
In the figure above, **we can see some interesting similarities and differences between the wheat and maize response to mean precipitation over the growing season. Of particular interest is between the mean and two standard deviations above the mean. The likelihood that substantial losses are reported doesn't increase meaningfully until over +2sd above the mean. In comparison, maize losses increased rapidly at +1sd. Past 3 standard deviation the likelihood of lossess increases rapidly. Would likely be linked to losses from rust or mold, or physical damage from storms all of which increase with excess rainfall.** Similarly, we can look at the partial dependence plot for other variables of interest such as `r thrd_imp_wht`.
...
#### Predict damages Sorghum  ----------------------------------------------------
 ...
 # read data
  ...
  data_in = data_in[!(data_in$EACODE %in% counts$EACODE),]
   ...
   #set as < because caret reverses the outcome variable 
sor_dam1_w_ea = factor(SORGHUMDAMAGEAREA_P<0.2) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  #                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  # print(sor.rf)
  # save(sor.rf,file = './Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P_GT_.2_zone_v2.RData')
  load('./Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P_GT_.2_zone_v2.RData')
...
For the case of Sorghum, we see impressive identification of reported Sorghum crop losses with a recall rate of `r round(cm_sor$byClass['Recall'],2)`, indicating that `r round(cm_sor$byClass['Recall'],2)*100`% of all substantial loss cases were predicted in our completely independent *testing* data. In all, `r cm_sor$table['FALSE','FALSE']` out of `r cm_sor$table['FALSE','FALSE']+ cm_sor$table['TRUE','FALSE']` reported cases were predicted, and only `r cm_sor$table['FALSE','TRUE']` cases were falsely predicted as crop failure areas. 
...
bar_dam1_w_ea = factor(BARLEYDAMAGEAREA_P<0.2) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  #                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  # print(bar.rf)
  # save(bar.rf,file = './Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P_GT_.2_zone_v2.RData')
  load('./Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P_GT_.2_zone_v2.RData')
...
tef_dam1_w_ea = factor(TEFFDAMAGEAREA_P<0.2) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  #                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  # print(tef.rf)
  # save(tef.rf,file = './Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P_GT_.2_zone_v2.RData')
  load('./Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P_GT_.2_zone_v2.RData')
...
For the case of teff, we see relatively poor identification of reported losses with a recall rate of only `r round(cm_tef$byClass['Recall'],2)`, indicating that only`r round(cm_tef$byClass['Recall'],2)*100`% of all substantial loss cases were predicted in our independent *testing* data. In all, `r cm_tef$table['FALSE','FALSE']` out of `r cm_tef$table['FALSE','FALSE']+ cm_tef$table['TRUE','FALSE']` reported cases were predicted, and  `r cm_tef$table['FALSE','TRUE']` cases were falsely predicted as crop failure areas. 
   

## Predict damages out of sample 
```{r Predict Damages to all EAs, eval=FALSE, fig.height=7, message=FALSE, warning=FALSE, include=FALSE}
  # Read in all EA data
  setwd(Home_dir)  
  EAs_poly = st_read("../Data/EnumerationAreas/EnumerationAreasSIN.shp") # use EA_code_merge as unique id
  Reg_poly = st_read("../Data/AdminBoundaries/ETH_adm2_UTM.shp") # use EA_code_merge as unique id
  ...
  load('./Outputs4Pred/maz.rf.MAIZEDAMAGEAREA_P_GT.2_zone.RData')
  ...
  jnd.tbl = all_eas_dam_sor %>% select(matches('20|EA_merge')) %>% unique()%>% left_join( jnd.tbl, . , by ='EA_merge')
  jnd.tbl = all_eas_dam_bar %>% select(matches('20|EA_merge')) %>% unique()%>% left_join( jnd.tbl, . , by ='EA_merge')
  jnd.tbl = all_eas_dam_wht %>% select(matches('20|EA_merge')) %>% unique()%>% left_join( jnd.tbl, . , by ='EA_merge')
  jnd.tbl = all_eas_dam_maz %>% select(matches('20|EA_merge')) %>% unique()%>% left_join( jnd.tbl, . , by ='EA_merge')
  jnd.tbl = all_eas_dam_tef %>% select(matches('20|EA_merge')) %>% unique()%>% left_join( jnd.tbl, . , by ='EA_merge') %>% as.data.frame()
  ...
  plot_dam_bar =  ggplot() +  geom_sf(data=subset(EAs_point_bar, Successful_crop == T), aes(color = Successful_crop ),size=1, alpha=0.5) +
    geom_sf(data=subset(EAs_point_bar, Successful_crop == F),  aes(color = Successful_crop ),size=1, alpha=0.5) + 
     geom_sf(data=Reg_poly,  aes( ),fill = NA )+ 
      facet_wrap( ~ Year, ncol=4)+ggtitle('Barley: Predicted Damage > 20%') 
  saveRDS(plot_dam_bar,'./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_bar.rds')
  plot_dam_bar = readRDS('./Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_bar.rds')
  
  #plot(plot_dam_bar)
  ggsave(plot_dam_bar,"~/Documents/IFPRI_Ethiopia_Drought_2016/IFPRI_Ethiopia_Drought_2016/Writeup/IFPRI_Ethiopia_Drought_2/plot_dam_bar.png")
```
