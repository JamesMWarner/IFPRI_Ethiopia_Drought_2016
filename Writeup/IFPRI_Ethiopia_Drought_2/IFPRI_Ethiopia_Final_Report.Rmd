...
Looking at the final model we can see an impressive adjusted R^2 of `r   #as.numeric(round(summary(maz.re3)$r.squared[2],2))`. Residuals are also relatively small with 1st and 3rd quintiles spanning `r as.numeric(round(summary(maz.re3$residuals)[2],2))` and `r as.numeric(round(summary(maz.re3$residuals)[5],2))` quintals.These numbers are relatively impressive considering the highly-localized nature of these estimates (at the sub-kebele level). Looking more closely we can observe some important features. 

First, remotely sensed variables, while complex, have a few tangible interpretations. For instance, *G_mn* is positive and significant (p<`r paste(signif(summary(maz.re3)[1]$coefficients['G_mn',4],3))`) indicating that yields are higher if the mean greenness (NDVI) level during the Meher growing season is higher. Similarly, increases in the 90th percentile of greenness for the entire year (*A_Qnt*), corresponds to statistically significant higher yields (p<`r paste(signif(summary(maz.re3)[1]$coefficients['A_Qnt',4],3))`). Interestingly, the variable *A_mn*, which is the mean greenness level (NDVI) for the entire year, is negative and significant (p<`r paste(signif(summary(maz.re3)[1]$coefficients['A_mn',4],3))`). Already controlling for conditions during the Meher season, this might be an indication of two issues, a) a higher *A_mn* might indicate the failure to properly till between seasons, or b) complications of management in dual-crop areas, where planting occurs for both the minor (Belg) and major (Meher) rainy season. 

Looking at precipitation variables we can see that *PPT_G_AUC_Qnt* yields are higher (p<`r paste(signif(summary(maz.re3)[1]$coefficients['PPT_G_AUC_Qnt',4],3))`) in sub-kebeles where the 90th percentile of total rainfall during the Meher season is higher. The opposite is true (p<`r paste(signif(summary(maz.re3)[1]$coefficients['PPT_G_mx_Qnt',4],3))`) for areas with higher 90 percentile maximum rainfalls (*PPT_G_mx_Qnt*) likely reflecting the impact of flooding or excess rain events. Additionally, areas with higher minimum potential evaporation rates (*PET_A_min*: higher min temps, more sun) receive better yields (p<`r paste(signif(summary(maz.re3)[1]$coefficients['PET_A_min',4],3))`) while controlling for precip and greenness, and areas with above average total PET (*PET_G_AUC_diff_mn*) also receive yield benefits (p<`r paste(signif(summary(maz.re3)[1]$coefficients['PET_G_AUC_diff_mn',4],3))`). 

The AgSS data also provide some insights. We see scant evidence for yield improvements based on planted area alone (*MAIZEAREA* p<`r paste(signif(summary(maz.re2)[1]$coefficients['MAIZEAREA',4],3))`), but we do see the positive effects of fertilization rates (*Fert_Amt_Per_Area* p<`r paste(signif(summary(maz.re3)[1]$coefficients['Fert_Amt_Per_Area',4],3))`), the use of improved seeds (*MAIZEIMSEED* p<`r paste(signif(summary(maz.re3)[1]$coefficients['MAIZEIMSEED',4],3))`), and the area of land in the sub-kebele that has utilized extension agents (*MAIZEEXTAREA* p<`r paste(signif(summary(maz.re3)[1]$coefficients['MAIZEEXTAREA',4],3))`). Conversely, we see the substantial impact of the percent of reported crop damage (*MAIZEDAMAGEAREA_P*), with every additional one percent of land damaged corresponding to a `r paste(signif(summary(maz.re3)[1]$coefficients['MAIZEDAMAGEAREA_P',1],3))` unit loss in OPH (p<`r paste(signif(summary(maz.re3)[1]$coefficients['MAIZEDAMAGEAREA_P',4],3))`). 
...
#### Predict damages MAIZE ----------------------------------------------------
 ...
 # read data
  ...
  data_in$dependent = factor(data_in$MAIZEDAMAGEAREA_P>0.3, levels=c('TRUE','FALSE'))
  
  #lag(data_in_plm$MAIZEOPH_W, 1) + c(year)MAIZEDAMAGEAREA_P
 maz_dam1_w_ea = dependent ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  model_data = data_in[,names(data_in) %in%   c(all.vars(maz_dam1_w_ea),'EACODE')]   
  ...
  library(RANN)
  set.seed(123)
  pp.train = na.omit(predict(preProcess(train, method = "knnImpute",k=25),train))   #naomit bc binary dependent variable has a few missing 
  ...
  # maz.rf <- randomForest(maz_dam1_w_ea, na.omit(pp.train),mtry = 3)
  # varImpPlot(maz.rf)
  # maz.rf %>% partial(pred.var = 'G_Qnt') %>% autoplot(rug = F, train = pp.train,x)+ labs(y='Likelihood of loss',x = paste('G_Qnt', '\n (centered & scaled)'))
  ...
  cm_maz = confusionMatrix(data =  pp.test$pred, reference = (pp.test$dependent),positive='TRUE')
```
### Maize Damages
...
`r pander(cm_maz$table, caption = "TRUE = Reported damages greater than 30%")`
*Table `r paste(Table_number)`: Performance metrics for reported damages > 30% for Maize* `r  Table_number=Table_number+1`
`r pander(data.frame(Accuracy=round(cm_maz$overall['Accuracy'],2), Kappa=round(cm_maz$overall['Kappa'],2), Recall=round(cm_maz$byClass['Recall'],2),row.names ='Value'), caption = "TRUE = Reported damages greater than 30%")`
 
We see impressive identification of reported maize crop losses with a recall rate of `r round(cm_maz$byClass['Recall'],2)`, indicating that 88% of all substantial loss cases in our completely independent *testing* data. In all, `r cm_maz$table['TRUE','TRUE']` out of `r cm_maz$table['TRUE','TRUE']+ cm_maz$table['FALSE','TRUE']` reported cases were predicted, and only `r cm_maz$table['TRUE','FALSE']` cases were false positives. 
 
We can also examine the relative importance of each variable to each model, as well as its estimated non-linear relationship with the dependent variable through partial dependency plots. First, let's examine the relativeimportance of each variable in the random forest through the role it played in Gini coefficient loss.
...
```
From the figure above we can see that `r most_imp_maz` has the highest relative importance in the random forest model. `r most_imp_maz` is **measured as the 95th percentile NDVI value for the growing season. As such the highest NDVI value (controling for outliers due to noise) likely reflects a measure of plant health, with higher `r rownames(varImp_maz$importance)[which.max(varImp_maz$importance$Overall)]` values corresponding to strong plant health at some point in the growing season.** We can also see that the most important variables are largely comprised of variables summarizing NDVI, with the exception of **PPT_G_mn which is the mean value of precipitation for each 10 day period across the growing season.** In the middle, we see the importance of precipitation and potential evapotranspiration (largely related to heat), and near the bottom, comparisons to historical averages. 

We can now look at the partial dependence plots to help understand the non-linear response of reported damages to the most important variable `r most_imp_maz`.  
...
```{r Partial Dependency Maize, echo=FALSE, message=FALSE, warning=FALSE,fig.width=7}
maz.rf %>% partial(pred.var = most_imp_maz) %>% autoplot(rug = F, train = pp.train,x)+ labs(y='Likelihood of loss',x = paste(most_imp_maz, '\n (centered & scaled)'))
```
In the figure above, **we can see how the likelihood of reporting maize losses increases rapidly after one standard deviation below the mean value of `r most_imp_maz`, with the likelihood of losses declining at the upper end of the distribution.**

Similarly, we can look at the partial dependence plot for the second most important variable `r secd_imp_maz`.
...
```{r Partial Dependency Maize second most important , echo=FALSE, message=FALSE, warning=FALSE,fig.width=7}
maz.rf %>% partial(pred.var = secd_imp_maz) %>% autoplot(rug = F, train = pp.train,x)+ labs(x = paste(secd_imp_maz, '\n (centered & scaled)'))
```
**`r secd_imp_maz` is a measure of variability across the entire year. We see a near linear increase in the likelihood of damage starting from two standard deviations (sd) below the mean, with a peak near one sd above. After this point, we see a dramatic drop in the likelihood. This is likely related to the fact that a plot of healthy plants harvested (especially in Meher and Belg seasons), followed by tilling, will likely have the highest variance of all plots. ** 

```{r Predict Damages Wheat Regression, echo=FALSE, message=FALSE, warning=FALSE,fig.height=7}
...
#### Predict damages WHEAT ----------------------------------------------------
 ...
 # read data
  ...
  # create dependent variable 
  data_in$dependent = factor(data_in$WHEATDAMAGEAREA_P>0.3, levels=c('TRUE','FALSE'))
  
wht_dam1_w_ea = dependent ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  # limit to variables of interest by EA 
  model_data = data_in[,names(data_in) %in%   c(all.vars(wht_dam1_w_ea),'EACODE')]   
  num_eas <- floor(0.75 * length(unique(model_data$EACODE)))
  train_eas <- sample(unique(model_data$EACODE), size = num_eas)
  ...
  wht.rf<-train(wht_dam1_w_ea,data=pp.train,method="rf",
                 ...
                 prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  print(maz.rf)
  save(wht.rf,file = './Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P>.3_zone.RData')
  load('./Outputs4Pred/wht.rf.WHEATDAMAGEAREA_P>.3_zone.RData')
  
  pp.test$pred = predict(wht.rf,pp.test)
  #confusionMatrix(data = (as.numeric(as.character(pp.test$pred))>0), reference = (pp.test$MAIZEDAMAGE_DROUGHT_DUM>0), mode = "prec_recall")
  confusionMatrix(data =  pp.test$pred, reference = (pp.test$WHEATDAMAGEAREA_P>0.3), mode = "prec_recall",positive='TRUE')
```

```{r Predict Damages Sorghum Regression, echo=FALSE, message=FALSE, warning=FALSE,fig.height=7}
...
#### Predict damages Sorghum  ----------------------------------------------------
 ...
 # read data
  ...
  # create dependent variable 
  data_in$dependent = factor(data_in$SORGHUMDAMAGEAREA_P>0.3, levels=c('TRUE','FALSE'))
  
sor_dam1_w_ea = dependent ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  # limit to variables of interest by EA 
  model_data = data_in[,names(data_in) %in%   c(all.vars(sor_dam1_w_ea),'EACODE')]   
  num_eas <- floor(0.75 * length(unique(model_data$EACODE)))
  train_eas <- sample(unique(model_data$EACODE), size = num_eas)
  ...
  sor.rf<-train(sor_dam1_w_ea,data=pp.train,method="rf",
                  ...
                  prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  print(sor.rf)
  save(sor.rf,file = './Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P>.3_zone.RData')
  load('./Outputs4Pred/sor.rf.SORGHUMDAMAGEAREA_P>.3_zone.RData')
  
  pp.test$pred = predict(sor.rf, pp.test)
  #confusionMatrix(data = (as.numeric(as.character(pp.test$pred))>0), reference = (pp.test$MAIZEDAMAGE_DROUGHT_DUM>0), mode = "prec_recall")
  confusionMatrix(data =  pp.test$pred, reference = (pp.test$SORGHUMDAMAGEAREA_P>0.3), mode = "prec_recall",positive='TRUE')
```

```{r Predict Damages Barley Regression, echo=FALSE, message=FALSE, warning=FALSE,fig.height=7}
...
#### Predict damages BARLEY  ----------------------------------------------------
 ...
 # read data
  ...
  # create dependent variable 
  data_in$dependent = factor(data_in$BARLEYDAMAGEAREA_P>0.3, levels=c('TRUE','FALSE'))
  
bar_dam1_w_ea = dependent ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  # limit to variables of interest by EA 
  model_data = data_in[,names(data_in) %in%   c(all.vars(bar_dam1_w_ea),'EACODE')]   
  num_eas <- floor(0.75 * length(unique(model_data$EACODE)))
  train_eas <- sample(unique(model_data$EACODE), size = num_eas)
  ...
  bar.rf<-train(bar_dam1_w_ea,data=pp.train,method="rf",
                  ...
                  prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  print(bar.rf)
  save(bar.rf,file = './Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P>.3_zone.RData')
  load('./Outputs4Pred/bar.rf.BARLEYDAMAGEAREA_P>.3_zone.RData')
  
  pp.test$pred = predict(bar.rf, pp.test)
  #confusionMatrix(data = (as.numeric(as.character(pp.test$pred))>0), reference = (pp.test$MAIZEDAMAGE_DROUGHT_DUM>0), mode = "prec_recall")
  confusionMatrix(data =  pp.test$pred, reference = (pp.test$BARLEYDAMAGEAREA_P>0.3), mode = "prec_recall",positive='TRUE')
```
   
```{r Predict Damages Teff Regression, echo=FALSE, message=FALSE, warning=FALSE,fig.height=7}
...
#### Predict damages TEFF  ----------------------------------------------------
 ...
 # read data
  ...
  # create dependent variable 
  data_in$dependent = factor(data_in$TEFFDAMAGEAREA_P>0.3, levels=c('TRUE','FALSE'))
  
tef_dam1_w_ea = dependent ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
	PET_G_min + PET_G_mx + PET_G_AUC + PET_G_Qnt + PET_G_AUC2 + PET_G_AUC_leading + PET_G_AUC_trailing + PET_G_AUC_diff_mn + PET_G_AUC_diff_90th + PET_G_sd+ ETA_A_mn + ETA_A_min + ETA_A_max + ETA_A_AUC + ETA_A_Qnt + ETA_A_sd + ETA_G_mn + ETA_G_min + ETA_G_mx + ETA_G_AUC + ETA_G_Qnt + ETA_G_AUC2 +  ETA_G_AUC_leading + ETA_G_AUC_trailing + ETA_G_AUC_diff_mn + ETA_G_AUC_diff_90th + ETA_G_sd+ PPT_A_mn + PPT_A_max + PPT_A_sd +  PPT_G_mn + PPT_G_mx + PPT_G_AUC + PPT_G_Qnt + PPT_G_mx_Qnt + PPT_G_AUC_Qnt + PPT_G_AUC2 + PPT_G_AUC_leading + PPT_G_AUC_trailing + PPT_G_AUC_diff_mn + PPT_G_AUC_diff_90th + PPT_T_G_Qnt + PPT_G_sd+ZONECODE
  ...
  # limit to variables of interest by EA 
  model_data = data_in[,names(data_in) %in%   c(all.vars(tef_dam1_w_ea),'EACODE')]   
  num_eas <- floor(0.75 * length(unique(model_data$EACODE)))
  train_eas <- sample(unique(model_data$EACODE), size = num_eas)
  ...
  tef.rf<-train(tef_dam1_w_ea,data=pp.train,method="rf",
                  ...
                  prox=TRUE,allowParallel=TRUE,tuneGrid = expand.grid(mtry=seq(1,15,1)))
  print(tef.rf)
  save(tef.rf,file = './Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P>.3_zone.RData')
  load('./Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P>.3_zone.RData')
  
  pp.test$pred = predict(tef.rf, pp.test)
  #confusionMatrix(data = (as.numeric(as.character(pp.test$pred))>0), reference = (pp.test$MAIZEDAMAGE_DROUGHT_DUM>0), mode = "prec_recall")
  confusionMatrix(data =  pp.test$pred, reference = (pp.test$TEFFDAMAGEAREA_P>0.3), mode = "prec_recall",positive='TRUE')
```
