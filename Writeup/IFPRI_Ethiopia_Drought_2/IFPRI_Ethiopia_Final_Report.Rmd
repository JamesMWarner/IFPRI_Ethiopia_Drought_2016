...
#### Predict damages MAIZE ----------------------------------------------------
 ...
 maz_dam1_w_ea = factor(MAIZEDAMAGEAREA_P>0.3) ~  A_mn + A_min + A_max + A_AUC + A_Qnt + A_sd +  A_max_Qnt +  A_AUC_Qnt  + G_mn  + G_min + G_mx + G_AUC + G_Qnt + G_mx_Qnt + G_AUC_Qnt + G_AUC2 + G_AUC_leading + G_AUC_trailing + G_AUC_diff_mn + G_AUC_diff_90th+T_G_Qnt+G_sd + PET_A_mn + PET_A_min + PET_A_max + PET_A_AUC + PET_A_Qnt + PET_A_sd + PET_G_mn + 
  ...
  cm_maz = confusionMatrix(data =  pp.test$pred, reference = (pp.test$MAIZEDAMAGEAREA_P>0.3),positive='TRUE')
```
### Maize Damages
...
*Table `r paste(Table_number)`: Confusion matrix & performance metrics for reported damages > 30% for Maize* `r  Table_number=Table_number+1`
`r pander(cm_maz$table)`
...
maz.rf %>% partial(pred.var = most_imp_maz) %>% autoplot(rug = F, train = pp.train,x)+ labs(x = paste(most_imp_maz, '\n (centered & scaled)'))
  #maz.rf %>% partial(pred.var = "A_sd") %>% autoplot(rug = F, train = pp.train,x)+ labs(x = "A_sd (centered)")
```
