...
```{r Predict Damages to all EAs,  echo=FALSE, message=FALSE, warning=FALSE,fig.height=7, include=FALSE} 
  # Read in all EA data
   library(sf)
   setwd(Home_dir)  
   nc <- st_read("../Data/EnumerationAreas/EnumerationAreasSIN.shp")
   load('./Outputs4Pred/all_eas_NDVI_ET_panel_V4.RData')
   all_eas_data = get('output')
   rm(output)
   
   #load damage model
   load('./Outputs4Pred/tef.rf.TEFFDAMAGEAREA_P_GT.3_zone.RData')
   all_eas_data$pred_maz = predict(tef.rf, all_eas_data)
